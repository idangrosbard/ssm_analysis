...
from src.types import MODEL_ARCH_AND_SIZE, MODEL_SIZE_CAT
            ...
            if tab == HEATMAP_TEXTS.TAB_SELECT_COMBINATION:
                ...
                if selected_row_idx is not None:
                    ...
                    if selected_prompt_idx_new != selected_prompt_idx:
                        if st.button(
                            HEATMAP_TEXTS.BUT_SAVE_NEW_SELECTION(selected_prompt_idx, selected_prompt_idx_new)
                        ):
                ...
                if selected_prompt_idx is not None:
                    ...
                    st.dataframe(
                        (
                            model_evals.pipe(
                                lambda df: df[[col for col in df.columns if col not in COLUMNS.PROMPT_DATA_COLS]]
                            )
                        ),
                        hide_index=True,
                        column_config={
