...
def load_knowns() -> Dataset:
    if not PATHS.RAW_KNOWN_1000_DIR.exists():
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            knowns_df = pd.read_json(Path(tmpdirname) / "known_1000.json").set_index(
                "known_id"
            )
...
def load_splitted_counter_fact(split: TSplit = (SPLIT.TRAIN1,)) -> Dataset:
    ...
    if not splitted_path.exists():
        ...
        splitted_dataset = split_dataset(dataset_name, num_splits, split_ratio, seed)
        splitted_dataset.save_to_disk(splitted_path)

    data = load_from_disk(splitted_path)  # type: ignore
...
def load_dataset(dataset_args: DatasetArgs) -> DatasetDict:
    if dataset_args.name == DATASETS.KNOWN_1000:
        return load_knowns_pd()
