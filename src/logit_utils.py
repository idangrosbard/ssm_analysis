...
def find_token_range(tokenizer, token_array, substring, ) -> tuple[int, int]:
...
def get_num_to_masks(
    ...
    tok_start, tok_end = find_token_range(
        tokenizer, input_ids[0], prompt.subject
    )
    subject_tokens = list(range(tok_start, tok_end))
    if 0 in subject_tokens:
        first_token = True

    if knockout_src == TokenType.first:
        src_idx = [0]
    elif knockout_src == TokenType.last:
        src_idx = [last_idx]
    elif knockout_src == TokenType.subject:
        src_idx = subject_tokens
    elif knockout_src == TokenType.relation:
        src_idx = [i for i in range(last_idx + 1) if i not in subject_tokens]
    elif knockout_src == TokenType.context:
        src_idx = [i for i in range(subject_tokens[0])]
    else:
        assert_never(knockout_src)

    if knockout_target == TokenType.last:
        target_idx = [last_idx]
    elif knockout_target == TokenType.subject:
        target_idx = subject_tokens
    else:
        assert_never(knockout_target)
...
def get_prompt_row(
        data: TPromptData,
        prompt_idx: int) -> Prompt:
    return Prompt(
        prompt_row=data.loc[prompt_idx])
