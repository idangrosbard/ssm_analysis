...
def get_args():
    ...
    parser.add_argument(
        ...
        default="ZERO_ATTENTION",
    )
    parser.add_argument(
        "--interfere_target",
        type=str,
        choices=[str(target).split(".")[1] for target in KnockoutTarget] + [None],
        default=None,
    )
...
def main_binary_search(
    model_size: str = "2.8B",
    interefere_mode: KnockoutMode = KnockoutMode.ZERO_ATTENTION,
    interefere_target: KnockoutTarget = KnockoutTarget.ENTIRE_SUBJ,
    affected_outputs: KnockoutTarget = KnockoutTarget.LAST,
