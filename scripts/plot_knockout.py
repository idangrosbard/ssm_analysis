...
def get_attention_style():
    ...
    color = {
        KnockoutTarget.ENTIRE_SUBJ: rgb_to_str(PURPLE),
        KnockoutTarget.SUBJ_LAST: rgb_to_str(PURPLE),
        KnockoutTarget.FIRST: rgb_to_str(BLUE),
        KnockoutTarget.LAST: rgb_to_str(RED),
        KnockoutTarget.RANDOM: "gray",
        KnockoutTarget.RANDOM_SPAN: "gray",
        KnockoutTarget.ALL_CONTEXT: "black",
        KnockoutTarget.SUBJ_CONTEXT: rgb_to_str(BLUE),
    }
    dash = {
        KnockoutTarget.ENTIRE_SUBJ: "solid",
        KnockoutTarget.SUBJ_LAST: "dot",
        KnockoutTarget.FIRST: "dash",
        KnockoutTarget.LAST: "dash",
        KnockoutTarget.RANDOM: "dot",
        KnockoutTarget.RANDOM_SPAN: "solid",
        KnockoutTarget.ALL_CONTEXT: "solid",
        KnockoutTarget.SUBJ_CONTEXT: "solid",
    }
...
def plot_bin_search(performance: pd.DataFrame, fig: go.Figure, row: int) -> go.Figure:
    interesting_tests = [
        ...
        KnockoutTarget.SUBJ_LAST,
        KnockoutTarget.RANDOM_SPAN,
        ...
        for layer in curr["layer"].unique():
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    y=curr_layer["acc"],
                    mode="lines+markers",
                    line=dict(color=color[str(target)]),
                    name=f"{str(target)}",
                    legendgroup=str(target),
                    showlegend=first,
                    line_dash=dash[str(target)],
                ),
                row=row + 1,
                col=1,
            )
        ...
        for layer in curr["layer"].unique():
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    y=curr_layer["acc"],
                    mode="lines+markers",
                    line=dict(color=color[str(category)]),
                    name=f"{str(category)}",
                    legendgroup=str(category),
                    showlegend=first,
                    line_dash=dash[str(category)],
                )
            )
            first = False
    ...
    for i, affected_output in enumerate(performance["affected_outputs"].unique()):
        for target in tqdm(performance["knockout_inputs"].unique()):
            if str(target) not in interesting_tests:
                continue
            curr = performance[
                (performance["knockout_inputs"] == target) & (performance["affected_outputs"] == affected_output)
            ]
            first = True
...
def main():
    ...
    if args.knockout_mode == "attention":
        performance = performance.melt(
            id_vars=["layer", "knockout_inputs", "affected_outputs", "acc"],
            value_vars=["start_layer", "end_layer"],
            value_name="x",
        )

    elif args.knockout_mode == "ssm":
        performance = performance.melt(
            id_vars=["layer", "norm", "category", "acc"],
            value_vars=["start_layer", "end_layer"],
            value_name="x",
        )
    ...
    if args.knockout_mode == "attention":
        fig = make_subplots(
            rows=performance["affected_outputs"].nunique(),
            cols=1,
            subplot_titles=performance["affected_outputs"].unique(),
        )
    ...
    fig.update_layout(
        title="SSM Interference",
        template="plotly_white",
        xaxis_title="Layer",
        yaxis_title="Accuracy",
        legend_title="Knockout Target",
        font=dict(size=18),
        height=1600,
        width=1000,
    )
